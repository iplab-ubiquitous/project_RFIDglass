...
model_type = "KNN"   # モデルの種類　[ SVC, KNN, RF, NN ]
...
# 予測フェーズ
clf = joblib.load(('./learningModel/test' + model_type + "_" + model_version + '.pkl'))
...
sns.heatmap(cm_pd, annot=True, cmap="Oranges", ax=ax)
plt.savefig('./confusionMatrix/result/png/confusion_matrix_data_' + model_type + "_" + model_version + '.png')
with open('./confusionMatrix/result/csv/confusion_matrix_data_' + model_type + "_" + model_version + '.csv', 'w') as file:
    writer = csv.writer(file, lineterminator='\n')
    writer.writerows(cm_pd)
print(classification_report(test_label, pred))
print("正答率 = ", metrics.accuracy_score(test_label, pred))

modellog = Logput(model_type)
modellog.logput("program: datapredict.py\n")
modellog.logput("Use model: test" + model_type + "_" + model_version + ".pkl\n")
modellog.logput("TestData: testData_" + data_version + ".csv\n")
modellog.logput('Save: confusion_matrix_data_' + model_type + "_" + model_version + '.csv\n')
modellog.logput('Save: confusion_matrix_data_' + model_type + "_" + model_version + '.png\n')
